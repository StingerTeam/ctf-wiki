...
这样可以在判断的时候保证input和correct的值一样

如果后端强制修改两个值并且要求保持判断一样的的时候，可以使用以上方法

> 引用绕过时，数引用位置需要从头开始数。
> O:3:"BUU":2:{s:7:"correct";s:0:"";s:5:"input";s:0:"";} 
> 如这个序列，R:1指向的时O:3:"BUU"；R:2指向的是s:7:"correct"
...
相当有趣的是`'$a, $b', 'return "ln($a) + ln($b) = " . log($a * $b)'`这一块会像字符串拼接一样直接拼接起来。
